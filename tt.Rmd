...
```{r}
exp_bias=ggplot(temp1, aes(x = sample_size, y = bias, color = model)) +
  geom_point(size = 1) +
  geom_line(size = 1) +
  xlab("sample size") +
  labs(
...
```{r}
exp_mse=ggplot(temp1, aes(x = sample_size, y = mse, color = model)) +
  geom_point(size = 1) +
  geom_line(size = 1) +
  xlab("sample size") +
  labs(
  ...
  theme(plot.caption = element_text(hjust = 0.5, size = rel(1.2)))
exp_mse
ggsave(file="results/exp_mse.pdf",width=8,height=5)
```

```{r}
exp_var=ggplot(temp1, aes(x = sample_size, y = variance, color = model)) +
  geom_point(size = 1) +
  geom_line(size = 1) +
  xlab("sample size") +
  labs(
  ...
  theme(plot.caption = element_text(hjust = 0.5, size = rel(1.2)))
exp_var
ggsave(file="results/exp_var.pdf",width=8,height=5)
```